...
import {
  ...
  getCategories, createCategory, uploadProductImage
} from "../firebase/db";
...
export default function InventoryPage() {
  ...
  const viewType = queryParams.get('view') || 'stock';
  ...
  const [formData, setFormData] = useState({
    ...
    category_id: "", backorder_allowed: false, image_url: "",
  });
  const [uploadingImage, setUploadingImage] = useState(false);
  const [categoryForm, setCategoryForm] = useState({ name: "", description: "" });

  useEffect(() => {
    if (selectedLocationId) fetchProducts();
  }, [selectedLocationId]);

  useEffect(() => {
    if (selectedCompanyId) fetchCategories();
  }, [selectedCompanyId]);
  ...
  const resetForm = () => {
    setFormData({ name: "", sku: "", barcode: "", price: "", cost: "", stock: "", category_id: "", backorder_allowed: false, image_url: "" });
        ...
        <div className="flex">
          ...
          <div className="w-64 bg-white border-r p-4 h-[calc(100vh-280px)] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              ...
              <button className="text-gray-600 hover:text-gray-900"><FaEllipsisV /></button>
            </div>
            <button onClick={() => setShowCategoryModal(true)}
            ...
            <div className="space-y-1">
              <button onClick={() => setSelectedCategory(null)}
                className={`w-full text-left px-3 py-2 rounded-lg transition ${!selectedCategory ? 'bg-gray-100 text-gray-900 font-medium' : 'text-gray-700 hover:bg-gray-50'}`}>
              ...
              {categories.map((category) => {
                ...
                return (
                  <button key={category.id} onClick={() => setSelectedCategory(category.id)}
                    className={`w-full text-left px-3 py-2 rounded-lg transition relative ${selectedCategory === category.id ? 'bg-gray-100 text-gray-900 font-medium' : 'text-gray-700 hover:bg-gray-50'}`}>
                    {category.name} ({count})
                    {count === 0 && <span className="absolute right-2 top-2 w-2 h-2 bg-red-500 rounded-full"></span>}
                  </button>
                );
              })}
            </div>
          ...
          <div className="flex-1 p-6">
            <div className="mb-6 flex items-center gap-4">
              <div className="flex-1 max-w-md relative">
                ...
                <input type="text" placeholder="Search by product name, item code..."
            ...
            {loading && products.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {filteredProducts.map((product) => (
                  <div key={product.id} className="bg-white rounded-lg border border-gray-200 overflow-hidden hover:shadow-md transition group">
                    ...
                    <div className="p-4">
                      ...
                      <div className="mb-3">
                        <div className={`w-full px-3 py-1.5 border rounded-lg text-sm font-medium text-center ${product.stock > 0 ? 'border-green-300 bg-green-50 text-green-700' : 'border-red-300 bg-red-50 text-red-700'}`}>
                      ...
                      {product.backorder_allowed && (
                        <div className="mt-2 px-2 py-1 bg-orange-100 text-orange-700 text-xs rounded text-center">Backorder Allowed</div>
                      )}
                    </div>
                  </div>
                ))}
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="border-t pt-4">
                    ...
                    {formData.image_url && (
                      <div className="mt-3">
                        ...
                        <img src={formData.image_url} alt="Product preview" className="w-32 h-32 object-cover rounded border border-gray-300" onError={(e) => { e.target.style.display = 'none'; }} />
                      </div>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    ...
                    <div>
                      ...
                      <input type="number" step="0.01" value={formData.cost} onChange={(e) => setFormData({ ...formData, cost: e.target.value })} min="0"
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none" />
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-2">
                    <input type="checkbox" id="backorder" checked={formData.backorder_allowed}
                      onChange={(e) => setFormData({ ...formData, backorder_allowed: e.target.checked })}
                      className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" />
                    <label htmlFor="backorder" className="text-sm text-gray-700">Allow backorder (sell when out of stock)</label>
                  </div>
                ...
                <form onSubmit={handleCategorySubmit} className="space-y-4">
                  <div>
                    ...
                    <input type="text" value={categoryForm.name} onChange={(e) => setCategoryForm({ ...categoryForm, name: e.target.value })} required
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="e.g., Electronics, Clothing, etc." />
                  </div>
                  <div>
                    ...
                    <textarea value={categoryForm.description} onChange={(e) => setCategoryForm({ ...categoryForm, description: e.target.value })} rows="3"
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="Category description..." />
                  </div>
                  <div className="flex gap-3 pt-4">
                    <button type="button" onClick={() => { setShowCategoryModal(false); setCategoryForm({ name: "", description: "" }); }}
                      className="flex-1 px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition">Cancel</button>
                    <button type="submit" disabled={loading}
                      className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition disabled:bg-gray-400">
